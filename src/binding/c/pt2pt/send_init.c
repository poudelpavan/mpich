...
int MPI_Send_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request) __attribute__ ((weak, alias("PMPI_Send_init")));
#endif
...
int MPI_Send_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_SEND_INIT);
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_SEND_INIT);
...
#ifdef HAVE_ERROR_CHECKING
    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER, "**mpi_send_init", "**mpi_send_init %p %d %D %i %t %C %p", buf, count, datatype, dest, tag, comm, request);
#endif
