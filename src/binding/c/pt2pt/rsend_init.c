...
int MPI_Rsend_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request) __attribute__ ((weak, alias("PMPI_Rsend_init")));
#endif
...
int MPI_Rsend_init(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_RSEND_INIT);
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_RSEND_INIT);
...
#ifdef HAVE_ERROR_CHECKING
    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER, "**mpi_rsend_init", "**mpi_rsend_init %p %d %D %i %t %C %p", buf, count, datatype, dest, tag, comm, request);
#endif
