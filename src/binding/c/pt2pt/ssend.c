...
int MPI_Ssend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm) __attribute__ ((weak, alias("PMPI_Ssend")));
#endif
...
int MPI_Ssend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_SSEND);
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_SSEND);
...
#ifdef HAVE_ERROR_CHECKING
    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER, "**mpi_ssend", "**mpi_ssend %p %d %D %i %t %C", buf, count, datatype, dest, tag, comm);
#endif
