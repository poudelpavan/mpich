...
int MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request) __attribute__ ((weak, alias("PMPI_Irecv")));
#endif
...
int MPI_Irecv(void *buf, int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Request *request)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_IRECV);
    ...
    if (unlikely(source == MPI_PROC_NULL)) {
        MPIR_Request *request_ptr = MPIR_Request_create_complete(MPIR_REQUEST_KIND__RECV);
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_IRECV);
...
#ifdef HAVE_ERROR_CHECKING
    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER, "**mpi_irecv", "**mpi_irecv %p %d %D %i %t %C %p", buf, count, datatype, source, tag, comm, request);
#endif
