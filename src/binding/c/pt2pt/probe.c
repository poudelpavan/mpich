...
int MPI_Probe(int source, int tag, MPI_Comm comm, MPI_Status *status)
{
    ...
    MPIR_FUNC_TERSE_ENTER(MPID_STATE_MPI_PROBE);
  ...
  fn_exit:
    MPIR_FUNC_TERSE_EXIT(MPID_STATE_MPI_PROBE);
...
#ifdef HAVE_ERROR_CHECKING
    mpi_errno = MPIR_Err_create_code(mpi_errno, MPIR_ERR_RECOVERABLE, __func__, __LINE__, MPI_ERR_OTHER, "**mpi_probe", "**mpi_probe %i %t %C %p", source, tag, comm, status);
#endif
