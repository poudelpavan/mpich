                                              ...
                                              MPIDI_winattr_t winattr, MPIR_Request ** sigreq)
{
    ...
    if (origin_contig && target_contig && (origin_bytes <= MPIDI_OFI_global.max_buffered_write)) {
        ...
        MPIDI_OFI_CALL_RETRY(fi_inject_write(MPIDI_OFI_WIN(win).ep,
                                             ...
                                             target_mr.mr_key), vni_src, rdma_inject_write, FALSE);
        goto null_op_exit;
    }
    ...
    if (origin_contig && target_contig) {
        ...
        MPIDI_OFI_CALL_RETRY(fi_writemsg(MPIDI_OFI_WIN(win).ep, &msg, flags), vni_src, rdma_write, FALSE);
                                              ...
                                              MPIDI_winattr_t winattr, MPIR_Request ** sigreq)
{
    ...
    if (origin_contig && target_contig) {
        ...
        riov.key = target_mr.mr_key;
        MPIDI_OFI_INIT_CHUNK_CONTEXT(win, sigreq);
        MPIDI_OFI_CALL_RETRY(fi_readmsg(MPIDI_OFI_WIN(win).ep, &msg, flags), vni_src, rdma_write, FALSE);
        /* Complete signal request to inform completion to user. */
        MPIDI_OFI_sigreq_complete(sigreq);
        goto fn_exit;
    }
                                                           ...
                                                           MPIDI_winattr_t winattr)
{
    ...
    msg.desc = NULL;
    msg.iov_count = 1;
    msg.addr = MPIDI_OFI_av_to_phys(av, vni_src, vni_dst);
    msg.rma_iov = &targetv;
    msg.rma_iov_count = 1;
    msg.datatype = fi_dt;
    msg.op = fi_op;
    ...
    MPIDI_OFI_CALL_RETRY(fi_compare_atomicmsg(MPIDI_OFI_WIN(win).ep, &msg,
                                              &comparev, NULL, 1, &resultv, NULL, 1, 0), vni_src,
                         atomicto, FALSE);
  fn_exit:
                                                     ...
                                                     MPIR_Request ** sigreq)
{
    ...
    if (origin_contig && target_contig) {
        ...
        msg.data = 0;
        MPIDI_OFI_INIT_CHUNK_CONTEXT(win, sigreq);
        MPIDI_OFI_CALL_RETRY(fi_atomicmsg(MPIDI_OFI_WIN(win).ep, &msg, flags), vni_src /*vci */ ,
                             rdma_atomicto, FALSE);
        /* Complete signal request to inform completion to user. */
        MPIDI_OFI_sigreq_complete(sigreq);
        goto fn_exit;
                                                         ...
                                                         MPIR_Request ** sigreq)
{
    ...
    if (origin_contig && target_contig && result_contig) {
        ...
        MPIDI_OFI_CALL_RETRY(fi_fetch_atomicmsg(MPIDI_OFI_WIN(win).ep, &msg, &resultv,
                                                NULL, 1, flags), vni_src /*vci */ , rdma_readfrom, FALSE);
        /* Complete signal request to inform completion to user. */
        MPIDI_OFI_sigreq_complete(sigreq);
        goto fn_exit;
    }
  ...
  null_op_exit:
    mpi_errno = MPI_SUCCESS;
    if (sigreq) {
        *sigreq = MPIR_Request_create_from_pool(MPIR_REQUEST_KIND__RMA, vni_src);
                                                       ...
                                                       MPIDI_winattr_t winattr)
{
    ...
    MPIDI_OFI_win_cntr_incr(win);
    MPIDI_OFI_CALL_RETRY(fi_fetch_atomicmsg(MPIDI_OFI_WIN(win).ep, &msg, &resultv,
                                            NULL, 1, 0), vni_src, rdma_readfrom, FALSE);
