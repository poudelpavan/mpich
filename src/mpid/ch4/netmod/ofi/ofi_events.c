...
static int am_isend_event(struct fi_cq_tagged_entry *wc, MPIR_Request * sreq, int vci);
...
#define MAX_HDR_SIZE 256        /* need accommodate MPIDI_AMTYPE_RDMA_READ */
    ...
    expected_seqno = MPIDI_OFI_am_get_next_recv_seqno(am_hdr->fi_src_addr, vci);
  ...
  fn_repeat:
    ...
    MPIDI_OFI_am_set_next_recv_seqno(fi_src_addr, next_seqno, vci);
...
int MPIDI_OFI_dispatch_function(struct fi_cq_tagged_entry *wc, MPIR_Request * req, int vci)
{
    ...
    if (MPIDI_OFI_REQUEST(req, event_id) == MPIDI_OFI_EVENT_SEND) {
        ...
        mpi_errno = am_isend_event(wc, req, vci);
        goto fn_exit;
        ...
        mpi_errno = am_isend_pipeline_event(wc, req, vci);
        goto fn_exit;
