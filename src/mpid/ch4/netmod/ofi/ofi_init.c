...
int init_am(int vci){
    ...
    if (MPIDI_OFI_ENABLE_AM) {
        ...
        for (int i = 0; i < MPIDI_OFI_NUM_AM_BUFFERS; i++) {
            ...
            MPIDI_OFI_global.am_list[vci].am_reqs[i].event_id = MPIDI_OFI_EVENT_AM_RECV;
            MPIDI_OFI_global.am_list[vci].am_reqs[i].index = i;
            ...
            MPIDI_OFI_global.am_list[vci].am_msg[i].addr = FI_ADDR_UNSPEC;
            MPIDI_OFI_global.am_list[vci].am_msg[i].context = &MPIDI_OFI_global.am_list[vci].am_reqs[i].context;
            MPIDI_OFI_global.am_list[vci].am_msg[i].iov_count = 1;
            MPIDI_OFI_CALL_RETRY(fi_recvmsg(MPIDI_OFI_global.ctx[ctx_idx].rx,
                                            &MPIDI_OFI_global.am_list[vci].am_msg[i],
                                            FI_MULTI_RECV | FI_COMPLETION), 0, prepost, FALSE);
        }

        MPIDIG_am_reg_cb(MPIDI_OFI_INTERNAL_HANDLER_CONTROL, NULL, &MPIDI_OFI_control_handler, vci);
        MPIDIG_am_reg_cb(MPIDI_OFI_AM_RDMA_READ_ACK, NULL, &MPIDI_OFI_am_rdma_read_ack_handler, vci);
    }
...
int MPIDI_OFI_mpi_finalize_hook(void)
{
    ...
    /* NOTE: am currently only use vni 0. Need update once that changes */
    for(int vci = 0; vci < MPIDI_OFI_global.num_vnis; vci++){
        while (MPL_atomic_load_int(&MPIDI_OFI_global.am_list[vci].am_inflight_inject_emus) > 0)
            MPIDI_OFI_PROGRESS(vci);
        MPIR_Assert(MPL_atomic_load_int(&MPIDI_OFI_global.am_list[vci].am_inflight_inject_emus) == 0);
    }
