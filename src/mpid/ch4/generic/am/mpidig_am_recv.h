...
MPL_STATIC_INLINE_PREFIX int MPIDIG_do_irecv(void *buf, MPI_Aint count, MPI_Datatype datatype,
                                             ...
                                             int alloc_req, uint64_t flags, int vni_src, int vni_dst)
{
    int mpi_errno = MPI_SUCCESS;
        ...
        if (MPIDIG_REQUEST(unexp_req, req->status) & MPIDIG_REQ_BUSY) {
            ...
            if (MPIDIG_REQUEST(unexp_req, recv_ready)) {
                ...
                MPIDIG_REQUEST(unexp_req, req->seq_no) =
                    MPL_atomic_fetch_add_uint64(&MPIDI_global.per_vci_list[vni_dst].nxt_seq_no, 1);
                MPL_DBG_MSG_FMT(MPIDI_CH4_DBG_GENERAL, VERBOSE,
                                (MPL_DBG_FDEST, "seq_no: me=%" PRIu64 " exp=%" PRIu64,
                                 MPIDIG_REQUEST(unexp_req, req->seq_no),
                                 MPL_atomic_load_uint64(&MPIDI_global.per_vci_list[vni_dst].exp_seq_no)));
                MPIDIG_recv_type_init(data_sz, unexp_req);
...
MPL_STATIC_INLINE_PREFIX int MPIDIG_mpi_irecv(void *buf,
                                              ...
                                              int is_local, MPIR_Request * partner, int vni_src, int vni_dst)
{
    int mpi_errno = MPI_SUCCESS;
    ...
    mpi_errno =
        MPIDIG_do_irecv(buf, count, datatype, rank, tag, comm, context_offset, request, 1, 0ULL, vni_src, vni_dst);
    MPIR_ERR_CHECK(mpi_errno);
  fn_exit:
