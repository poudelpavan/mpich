...
MPL_STATIC_INLINE_PREFIX MPIR_Comm *MPIDIG_context_id_to_comm(uint64_t context_id)
{
    ...
    ret = MPIDI_global.per_vci_list[0].comm_req_lists[comm_idx].comm[is_localcomm][subcomm_type];

    MPIR_FUNC_VERBOSE_EXIT(MPID_STATE_MPIDIG_CONTEXT_ID_TO_COMM);
    return ret;
}

MPL_STATIC_INLINE_PREFIX MPIDIG_rreq_t **MPIDIG_context_id_to_uelist(uint64_t context_id, int vci)
{
    int comm_idx = MPIDIG_get_context_index(context_id);
    int subcomm_type = MPIR_CONTEXT_READ_FIELD(SUBCOMM, context_id);
    int is_localcomm = MPIR_CONTEXT_READ_FIELD(IS_LOCALCOMM, context_id);
    ...
    ret = &MPIDI_global.per_vci_list[vci].comm_req_lists[comm_idx].uelist[is_localcomm][subcomm_type];
...
MPL_STATIC_INLINE_PREFIX bool MPIDIG_rma_need_poll_am(int vci)
{
    bool poll_flag = false;

    if (MPIR_CVAR_CH4_RMA_ENABLE_DYNAMIC_AM_PROGRESS) {
        int interval;
        MPIR_cc_incr(&MPIDIG_global.am[vci].rma_am_poll_cntr, &interval);

        /* Always poll if any RMA target-side AM has arrived because
         * we expect more incoming AM now. */
        if (MPL_atomic_load_int(&MPIDIG_global.am[vci].rma_am_flag)) {
            poll_flag = true;
        } else {
...
MPL_STATIC_INLINE_PREFIX void MPIDIG_rma_set_am_flag(int vci)
{
    MPL_atomic_store_int(&MPIDIG_global.am[vci].rma_am_flag, 1);
}
